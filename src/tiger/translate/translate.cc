...
public:
    type::Ty *ty_;
    llvm::Value *val_;
    llvm::BasicBlock *last_bb_; // 需要返回返回值所在的 basic block 以此来支持 phi 追溯正确的 basic block(嵌套的分支)
                        ...
                        err::ErrorMsg *errormsg) const
{
    ...
    // 对于所有 NameTy 要进行是否出现环的检测
    // 从每一个 NameTy 遍历 NameTy 形成的 Type 链
    // 如果遇到一个 NameTy 先前已经加入到集合中的 NameTy 说明出现了环
    // 不过环检测应该是在类型检查中做 这里只是简单复制了代码
                            ...
                            tr::Level *level, err::ErrorMsg *errormsg) const
{
    ...
    for (const auto &absyn_fun_dec : this->functions_->GetList()) {
        ...
        /* 保证每个函数中 %0 只被使用一次 即只在函数入口处用来计算栈帧指针 这样子在codeGen中可以直接跳过 */
                                 ...
                                 err::ErrorMsg *errormsg) const
{
    ...
    //寻找要传递给被调用函数的 static link
    while (current_level != callee_level->parent_) {    // 直到找到被调函数的祖先
    ...
    for (const auto &formal : this->args_->GetList()) {
        auto formal_val_ty = formal->Translate(venv, tenv, level, errormsg);    // 翻译每个参数表达式
                               ...
                               err::ErrorMsg *errormsg) const
{
    ...
    if (this->elsee_ && !else_body_ret_val_llvm->ty_->IsSameType(type::VoidTy::Instance())) {
        ...
        // 如果存在 record & nil
        // 只有可能一个是 nil
        auto ret_type = then_body_ret_val_llvm->ty_->ActualTy() == type::NilTy::Instance() ? else_body_ret_val_llvm->ty_->ActualTy() : then_body_ret_val_llvm->ty_->ActualTy();

        auto then_body_ret_val = then_body_ret_val_llvm->ty_->ActualTy() == type::NilTy::Instance() ? llvm::ConstantPointerNull::get(static_cast<llvm::PointerType *>(else_body_ret_val_llvm->ty_->GetLLVMType())) : then_body_ret_val_llvm->val_;
        ...
        auto else_body_ret_val = else_body_ret_val_llvm->ty_->ActualTy() == type::NilTy::Instance() ? llvm::ConstantPointerNull::get(static_cast<llvm::PointerType *>(then_body_ret_val_llvm->ty_->GetLLVMType())) : else_body_ret_val_llvm->val_;
